...
FOUND
	...
	CP	'?'		;Help requested?
	JP	Z,HELP		;Jump if yes
	CP	'-'		;Help requested? e.g. -? or -H
	JP	Z,HELP		;Jump if yes
	...
	LD	HL,SWNSET	;then check to se that the
	CP	OSCON		;SmartWatch is already set, if
	JP	Z,MSGEXT	;if it is no set, then print an error
	...
	JP	Z,TURNOFF	;Jump if yes
	CP	'H'		;Help requested?
	JP	Z,HELP		;Jump if yes
	OR	A		;Any other parameter goes to HELP
	JR	Z,PRTTIM	;No parameter; Go show time, then back to DOS
HELP				;Print Help.
...
;
HLPMSG
	DB	'This program allows setting and reading of the Dallas Semiconductor ',LF
	DB	'DS1315 Phantom Clock and DS 1216E SmartWatch.',LF
;	DB	'in a TRS-80 Model II.',LF
	DB	'CLK2 T <CR> will set the system date and time '
	DB	'from the SmartWatch.',LF
	DB	'CLK2 O <CR> turn the SmartWatch off, for long term storage.',LF
	DB	'CLK2 S hhmmssMMDDYYdm<CR> will set the time, '
	DB	'date and day in the SmartWatch.',LF,LF
	DB	'Where:',LF
...
TIMBUF$	DB	'hh:mm:ss'
DATBUF$	DB	'MM/DD/YY'
FREMSG	DB	'Shared Copyright 1987 Duane M. Saylor',LF
	DB	'Shared Copyright 2025, Ruud Broers',CR
	DB	'Version 03/09/2025',CR
TIMSTR$	DC	128,'S'
STACK$	DS	2
LSTADR	EQU	$
	END	START
